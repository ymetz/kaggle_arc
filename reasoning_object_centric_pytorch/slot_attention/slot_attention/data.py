...
class CLEVERERDataset(Dataset):
    def __init__(
        ...
        self.split = split
        assert os.path.exists(self.data_root), f"Path {self.data_root} does not exist"
        assert self.split == "train" or self.split == "validation" or self.split == "test"
        assert os.path.exists(self.data_path), f"Path {self.data_path} does not exist"
        self.files = self.get_files()
    ...
    def get_files(self) -> List[str]:
        with open(os.path.join(self.data_root, f"summaryJSON_{self.split}.json")) as f:
            scene = json.load(f)
        paths: List[Optional[str]] = []
        total_num_videos = len(scene.keys())
        # Calculate how many images per video we sample, fill up with rest_num_images to get desired dataset size
        if self.max_num_images is not None:
            max_images_per_scene = math.floor(self.max_num_images / total_num_videos)
            rest_num_images = self.max_num_images - max_images_per_scene * total_num_videos
        else:
            max_images_per_scene = int(1e7) # unreasonably high value -> min operation always outputs frames for current video
            rest_num_images = 0
        i = 0
        rest_idx = 0
        while (self.max_num_images is None or len(paths) < self.max_num_images) and i < total_num_videos:
            num_objects_in_scene = len(list(scene.values())[i]["object_property"])
            if num_objects_in_scene <= self.max_n_objects:
                max_frames_for_current_video = len([1 for x in list(os.scandir(os.path.join(self.data_root, f"{self.split}/{list(scene.values())[i]['output_path']}"))) if x.is_file()])
                if rest_idx < rest_num_images:
                    max_images_per_scene = max_images_per_scene + 1
                    rest_idx += 1
                for img_idx in range(min(max_images_per_scene, max_frames_for_current_video)):
                    image_path = os.path.join(self.data_root, f"{self.split}/{list(scene.values())[i]['output_path']}", f"frame_{str(img_idx).zfill(5)}.png")
                    assert os.path.exists(image_path), f"{image_path} does not exist"
                    paths.append(image_path)        
            i += 1
        return sorted(compact(paths))
...
class CLEVERERDataModule():
    def __init__(
        ...
        self.train_dataset = CLEVERERDataset(
            data_root=self.data_root,
            max_num_images=self.num_train_images,
            clevr_transforms=self.clevr_transforms,
            split="train",
        ...
        self.val_dataset = CLEVERERDataset(
            data_root=self.data_root,
            max_num_images=self.num_val_images,
            clevr_transforms=self.clevr_transforms,
            split="validation",
